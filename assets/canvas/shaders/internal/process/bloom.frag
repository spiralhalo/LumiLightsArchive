...
vec4 l2_ssr_reflection(float water){
	...
	float sideCoord = _cvv_texcoord.x+(_cvv_texcoord.x-0.5)*(upCoord-groundCoord);
...
void main() {
	...
	if (water > 0){
		gl_FragData[0] = bloomResult + (1-water)*l2_ssr_reflection(water) * 0.8;
	} else {
